"use client";

import Link from "next/link";
import { useEffect, useMemo, useState } from "react";
import { CalendarDays, CloudSun, ExternalLink, Lock, Plus, Unlock } from "lucide-react";
import { CalendarGrid, type DayCell } from "@/components/admin/calendar-grid";
import { DayActionsModal } from "@/components/admin/day-actions-modal";
import { StatusBadge } from "@/components/admin/status-badge";
import {
  AVAILABILITY_STATUS_LABEL,
  BOOKING_SOURCE_LABEL,
  labelOrRaw,
} from "@/lib/admin/labels";
import { formatItDay } from "@/lib/dates";
import type { DayCellEnriched } from "./enrich";

export interface CalendarWeatherSummary {
  date: string;
  risk: string;
  reasons: string[];
  forecast: {
    temperatureMax: number;
    temperatureMin: number;
    windSpeedKmh: number;
    windGustKmh: number;
    precipitationProbability: number;
    precipitationMm: number;
    waveHeightM: number | null;
  };
}

export interface CalendarBoatView {
  boatId: string;
  boatName: string;
  days: DayCell[];
  enriched: DayCellEnriched[];
}

export interface CalendarClientProps {
  calendars: CalendarBoatView[];
  weather: CalendarWeatherSummary[];
  initialSelected?: { boatId: string; dateIso: string } | null;
}

export function CalendarClient({ calendars, weather, initialSelected }: CalendarClientProps) {
  const fallbackSelected = useMemo(() => {
    if (initialSelected) return initialSelected;
    const firstCalendar = calendars[0];
    const firstDay = firstCalendar?.enriched[0];
    return firstCalendar && firstDay
      ? { boatId: firstCalendar.boatId, dateIso: firstDay.dateIso }
      : null;
  }, [calendars, initialSelected]);

  const [selected, setSelected] = useState<{ boatId: string; dateIso: string } | null>(fallbackSelected);
  const [actionDay, setActionDay] = useState<{
    boatId: string;
    boatName: string;
    day: DayCellEnriched;
  } | null>(null);

  const enrichedByKey = useMemo(() => {
    const map = new Map<string, { boatId: string; boatName: string; day: DayCellEnriched }>();
    for (const calendar of calendars) {
      for (const day of calendar.enriched) {
        map.set(`${calendar.boatId}|${day.dateIso}`, {
          boatId: calendar.boatId,
          boatName: calendar.boatName,
          day,
        });
      }
    }
    return map;
  }, [calendars]);

  const weatherByDate = useMemo(() => {
    const map = new Map<string, CalendarWeatherSummary>();
    for (const item of weather) map.set(item.date, item);
    return map;
  }, [weather]);

  const dataVersion = useMemo(
    () =>
      calendars
        .flatMap((calendar) =>
          calendar.enriched.map(
            (day) => `${calendar.boatId}:${day.dateIso}:${day.status}:${day.bookings.length}`,
          ),
        )
        .join("|"),
    [calendars],
  );

  useEffect(() => {
    setSelected(fallbackSelected);
    setActionDay(null);
  }, [dataVersion, fallbackSelected]);

  const selectedRecord = selected
    ? enrichedByKey.get(`${selected.boatId}|${selected.dateIso}`) ?? null
    : null;
  const selectedWeather = selectedRecord
    ? weatherByDate.get(selectedRecord.day.dateIso)
    : undefined;

  return (
    <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_420px]">
      <div className="space-y-5">
        {calendars.length === 0 ? (
          <p className="rounded-xl border border-slate-200 bg-white p-6 text-sm text-slate-500">
            Nessuna barca o esperienza trovata per questo filtro.
          </p>
        ) : (
          calendars.map((calendar) => (
            <div key={calendar.boatId} className="rounded-xl border border-slate-200 bg-white p-4">
              <CalendarGrid
                days={calendar.days}
                boatName={calendar.boatName}
                boatId={calendar.boatId}
                onDayClick={(dateIso) => setSelected({ boatId: calendar.boatId, dateIso })}
              />
            </div>
          ))
        )}
      </div>

      <aside className="xl:sticky xl:top-20 xl:self-start">
        <DayDetail
          selected={selectedRecord}
          weather={selectedWeather}
          onOpenActions={(payload) => setActionDay(payload)}
        />
      </aside>

      {actionDay && (
        <DayActionsModal
          boatId={actionDay.boatId}
          boatName={actionDay.boatName}
          day={actionDay.day}
          onClose={() => setActionDay(null)}
        />
      )}
    </div>
  );
}

function DayDetail({
  selected,
  weather,
  onOpenActions,
}: {
  selected: { boatId: string; boatName: string; day: DayCellEnriched } | null;
  weather?: CalendarWeatherSummary;
  onOpenActions: (payload: { boatId: string; boatName: string; day: DayCellEnriched }) => void;
}) {
  if (!selected) {
    return (
      <div className="rounded-xl border border-slate-200 bg-white p-5 text-sm text-slate-500">
        Seleziona un giorno nel calendario per vedere dettagli e azioni.
      </div>
    );
  }

  const { boatId, day, boatName } = selected;
  const statusClass =
    day.status === "BLOCKED"
      ? "border-red-200 bg-red-50 text-red-800"
      : day.status === "PARTIALLY_BOOKED"
        ? "border-amber-200 bg-amber-50 text-amber-800"
        : "border-emerald-200 bg-emerald-50 text-emerald-800";

  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-slate-500">
            <CalendarDays className="size-4" aria-hidden="true" />
            {boatName}
          </p>
          <h2 className="mt-2 text-xl font-bold text-slate-950">{formatItDay(day.date)}</h2>
        </div>
        <span className={`rounded-full border px-2.5 py-1 text-xs font-semibold ${statusClass}`}>
          {labelOrRaw(AVAILABILITY_STATUS_LABEL, day.status)}
        </span>
      </div>

      <div className="mt-5 space-y-5">
        <WeatherPanel weather={weather} />

        <section className="space-y-3">
          <h3 className="text-sm font-semibold text-slate-900">Prenotazioni del giorno</h3>
          {day.bookings.length === 0 ? (
            <p className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-500">
              Nessuna prenotazione su questa data.
            </p>
          ) : (
            <div className="space-y-2">
              {day.bookings.map((booking) => (
                <Link
                  key={booking.id}
                  href={`/admin/prenotazioni/${booking.id}`}
                  className="block rounded-lg border border-slate-200 p-3 text-sm hover:bg-slate-50"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="font-mono font-semibold text-blue-700">
                        {booking.confirmationCode}
                      </p>
                      <p className="mt-1 truncate font-medium text-slate-900">
                        {booking.serviceName}
                      </p>
                      <p className="mt-1 truncate text-xs text-slate-500">
                        {booking.customerName} · {labelOrRaw(BOOKING_SOURCE_LABEL, booking.source)}
                      </p>
                    </div>
                    <StatusBadge status={booking.status} kind="booking" />
                  </div>
                </Link>
              ))}
            </div>
          )}
        </section>

        <section className="space-y-3">
          <h3 className="text-sm font-semibold text-slate-900">Azioni rapide</h3>
          <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">
            <button
              type="button"
              onClick={() => onOpenActions({ boatId, boatName, day })}
              className="inline-flex items-center justify-center gap-2 rounded-lg bg-slate-950 px-3 py-2 text-sm font-medium text-white hover:bg-slate-800"
            >
              {day.status === "BLOCKED" ? (
                <Unlock className="size-4" aria-hidden="true" />
              ) : (
                <Lock className="size-4" aria-hidden="true" />
              )}
              {day.status === "BLOCKED" ? "Sblocca" : "Blocca giorno"}
            </button>
            <Link
              href={`/admin/prenotazioni?date=${encodeURIComponent(day.dateIso)}`}
              className="inline-flex items-center justify-center gap-2 rounded-lg border border-slate-300 px-3 py-2 text-sm font-medium hover:bg-slate-50"
            >
              <ExternalLink className="size-4" aria-hidden="true" />
              Apri prenotazioni
            </Link>
            <Link
              href={`/admin/prenotazioni?date=${encodeURIComponent(day.dateIso)}`}
              className="inline-flex items-center justify-center gap-2 rounded-lg border border-slate-300 px-3 py-2 text-sm font-medium hover:bg-slate-50 sm:col-span-2 xl:col-span-1 2xl:col-span-2"
            >
              <Plus className="size-4" aria-hidden="true" />
              Nuova prenotazione
            </Link>
          </div>
        </section>
      </div>
    </div>
  );
}

function WeatherPanel({ weather }: { weather?: CalendarWeatherSummary }) {
  if (!weather) {
    return (
      <section className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-500">
        <h3 className="font-semibold text-slate-900">Meteo</h3>
        <p className="mt-1">Dati non disponibili per questa data.</p>
      </section>
    );
  }

  return (
    <section className="rounded-lg border border-slate-200 p-3">
      <div className="flex items-center justify-between gap-3">
        <h3 className="flex items-center gap-2 text-sm font-semibold text-slate-900">
          <CloudSun className="size-4 text-slate-500" aria-hidden="true" />
          Meteo
        </h3>
        <span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-700">
          {weather.risk}
        </span>
      </div>
      <dl className="mt-3 grid grid-cols-2 gap-2 text-xs text-slate-600">
        <Metric label="Vento" value={`${Math.round(weather.forecast.windSpeedKmh)} km/h`} />
        <Metric label="Raffiche" value={`${Math.round(weather.forecast.windGustKmh)} km/h`} />
        <Metric label="Onde" value={weather.forecast.waveHeightM === null ? "n/d" : `${weather.forecast.waveHeightM.toFixed(1)} m`} />
        <Metric label="Pioggia" value={`${weather.forecast.precipitationProbability}%`} />
      </dl>
      {weather.reasons[0] && <p className="mt-3 text-xs text-slate-500">{weather.reasons[0]}</p>}
    </section>
  );
}

function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <dt>{label}</dt>
      <dd className="font-semibold tabular-nums text-slate-900">{value}</dd>
    </div>
  );
}
