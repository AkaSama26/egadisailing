...
export function overbookingApologyTemplate(data: OverbookingApologyData) {
  ...
  const phoneLine = data.contactPhone
    ? `<p><strong>${locale === "en" ? "Phone/WhatsApp" : locale === "es" ? "Teléfono/WhatsApp" : locale === "fr" ? "Téléphone/WhatsApp" : "Telefono/WhatsApp"}:</strong> ${escapeHtml(data.contactPhone)}</p>`
    : "";
  ...
  const rebookingHtml =
    data.rebookingSuggestions && data.rebookingSuggestions.length > 0
      ? `<p>${locale === "en" ? "Alternative dates" : locale === "es" ? "Fechas alternativas" : locale === "fr" ? "Dates alternatives" : "Date alternative"}:<br>${data.rebookingSuggestions.map((d) => escapeHtml(d)).join("<br>")}</p>`
      : "";
  ...
  const html = emailLayout({
    ...
    bodyHtml: `
      ...
      <p><strong>${locale === "en" ? "Direct email" : locale === "es" ? "Email directo" : locale === "fr" ? "Email direct" : "Email diretta"}:</strong> <a href="mailto:${escapeHtml(data.contactEmail)}">${escapeHtml(data.contactEmail)}</a></p>
      ${phoneLine}
  ...
  const rebookingText =
    data.rebookingSuggestions && data.rebookingSuggestions.length > 0
      ? `\n\n${locale === "en" ? "Alternatives" : locale === "es" ? "Alternativas" : locale === "fr" ? "Alternatives" : "Alternative"}: ${data.rebookingSuggestions.join(", ")}`
      : "";
